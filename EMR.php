	...
	*/
	function pagarboleto($horariotransbordo, $colectivo){
		if($this->tipo == 1){ 											//tipo 1 = tarjeta comun
			if(date("d.m.y H:i:s") < $horariotransbordo && $colectivo->linea!=$this->ultlinea && $this->count < 2){	// date(..) = hora actual
				if($this->saldo >= 1.90){
					$this->saldo = $this->saldo - 1.90;
					$this->viajes[]= new viajes(date("d.m.y H:i:s"),$colectivo->linea,1.90);
				...
				if($this->saldo >= 5.75){
					...
					$this->viajes[]= new viajes(date("d.m.y H:i:s"),$colectivo->linea,5.75);
					$this->ultlinea = $colectivo->linea;
		...
		if(date("H:i:s") > strtotime("00:00:00") and date("H:i:s") < strtotime("06:00:00")){  //chequea si esta en horario funcional
			if(date("d.m.y H:i:s") < $horariotransbordo && $colectivo->linea != $this->ultlinea && $this->count < 2){
				if($this->saldo >= 1.90){
					$this->saldo = $this->saldo - 1.90;
					$this->viajes[]= new viajes(date("d.m.y H:i:s"),$colectivo->linea,1.90);
					echo "Disfrute su viaje transbordo";
			...
			if(date("d.m.y H:i:s") < $horariotransbordo && $colectivo->linea != $this->ultlinea && $this->count < 2){
				if($this->saldo >= 0.96 ){
					...
					$this->viajes[]= new viajes(date("d.m.y H:i:s"),$colectivo->linea,0.96);
					echo "Disfrute su viaje transbordo";
					$this->ultlinea = $colectivo->linea;
					$this->count++;
					return true;
				...
				if($this->saldo >= 2.90){
					...
					$this->viajes[]= new viajes(date("d.m.y H:i:s"),$colectivo->linea,2.90);
					$this->ultlinea = $colectivo->linea;
					echo "Disfrute su viaje";
					$this->count=0;
					return true;
